...
height = Number(height);
  ...
  else if(allowFullScreen == true){
    return '<iframe src="'+src+'" height="'+height+'" allowfullscreen></iframe>';
  }
  else if(height<0 ){
    return '<iframe src="'+src+'"></iframe>';
  }
  else{
    return '<iframe src="'+src+'" height="'+height+'"></iframe>';
  }
...
function fixPostalCode(postalCode) {
  var i=0;
  var a=0;
  var arr1= ['D', 'F', 'I', 'O', 'Q', 'W', 'U', 'Z'];
  var arr2 = ['D', 'F', 'I', 'O', 'Q', 'U'];
  ...
  if (/[^DFIOQWUZ\d]\d[^DFIOQU\d]\s\d[^DFIOQU\d]\d/.test(postalCode) === false) {
...
function normalizeCoord(value) {
 ...
 if(value.match(/,\s/)) {
   storage = value.split(', ');
   if (storage[0] < -90 || storage[0] > 90 || storage[1] < -180 || storage[1] > 180) {
     normalizeCoord(value).toThrow();
   } else {
     store='(' + storage[0] + ', ' + storage[1] + ')';
     return store;
   }
 }
 else if(value.match(/\[/)){
   storage = value.split(',');
   storage[0] = storage[0].slice(1);
   storage[1] = storage[1].slice(0, -1);
   if (storage[1] < -90 || storage[1] > 90 || storage[0] < -180 || storage[0] > 180){
     normalizeCoord(value).toThrow();
   }else{
     store='(' + storage[1] + ', ' + storage[0] + ')';
     return store;
   }
 }
 else{
...
function formatCoords(...values) {
  ...
  {
    try {
      correct=normalizeCoord(values[z]);
...
function countForProvince(provinceCode, ...postalCodes) {
  if (postalCodes.length == 0) {
    throw 'Postal code not entered/given.';
  }
  else {
    var counter = 0;
    postalCodes.forEach((postalCode) => {
      ...
      else if (toProvince(postalCode, true) === provinceCode) {
        counter++;
      }
    });
    return counter;
...
function generateLicenseLink(licenseCode, targetBlank) {
  var link;
  var arr1 = ['by', 'by-nc', 'by-sa', 'by-nd', 'by-nc-sa', 'by-nc-nd'];
  var arr2 = [
  ...
  if (licenseCode != null && licenseCode.length > 3) {
    licenseCode = licenseCode.slice(3);
    licenseCode = licenseCode.toLowerCase();
  }
//Doing above steps so that my licenseCode becomes equal to arr1
  if (arr1.includes(licenseCode)) {
    for (let i = 0; i < arr1.length; i++) {
      if (licenseCode == arr1[i] && targetBlank) {
        link = `<a href="https://creativecommons.org/licenses/${arr1[i]}/4.0/" target="_blank">${arr2[i]}</a>`;
      }
      if (licenseCode == arr1[i] && !targetBlank) {
        link = `<a href="https://creativecommons.org/licenses/${arr1[i]}/4.0/">${arr2[i]}</a>`;
      }
    }
  } else {
    link = '<a href="https://choosealicense.com/no-permission/">All Rights Reserved</a>';
  }
  return link;
}
...
function buildUrl(query, order, count, license) {
  ...
  let updateQuery = encodeURIComponent(query);
  let sortOrder = order;
  ...
  if (!licen.includes(licenseCode)) {
    throw 'Invalid license.';
  }

  return `https://api.inaturalist.org/v2/observations?query=${updateQuery}&count=${calc}&order=${sortOrder}&license=${licenseCode}`;
}
